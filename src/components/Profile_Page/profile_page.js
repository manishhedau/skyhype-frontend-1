            ...
            <div className="user-profile">
                <SwipeableViews enableMouseEvents>
                    <div className="profile-container">
                        <div className="profile-page" style={{ background: userInfo.styles.background, fontFamily: userInfo.styles.font }}>
                            ...
                            <div className="profile-info">
                                ...
                                <div className="social-media-links">
                                    {social_link_list.map((link, index) => (
                                        link.isActive && <a key={index} href={iconsObject[link.social_media].baseURL + link.link} onClick={() => onSocialLinkClickHandler(link)} target="_blank" rel="noreferrer"><i className={`fab ${iconsObject[link.social_media].icon}`}></i></a>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="activity-links profile-activity-links">
                                {restActivityLink.map((link, index) => (
                                    index !== 10 ? <ActivityTile key={index} thumbnail="https://miro.medium.com/max/4800/1*scwcpro6eU-pORVH_Fpc_g.png" title={link.title} onClick={() => onActivityLinkClickHandler(link)} /> : ""
                                ))}
                            </div>
                            ...
                            {!isJoinClub && <button className="btn btn-primary join-club-button" onClick={handleJoinMyClubClick}> Join my club</button>}
                            {/* </div> */}
