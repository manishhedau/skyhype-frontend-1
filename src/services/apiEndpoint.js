...
const API = axios.create({ baseURL: configration.backendURI });
...
export const callIPInfo = async () => {
    try {
        ...
        const res = await axios.get(configration.ipInfo)
...
export const getSpotlightData = async (userId) => {

    try {
        const data = await API.get(`dashboard/update-activity-links/spotlight/${userId}`);
        const ret = await data.data;
        console.log(ret);
        return ret;
    }
